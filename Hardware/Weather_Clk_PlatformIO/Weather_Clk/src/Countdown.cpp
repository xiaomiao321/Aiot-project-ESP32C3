...
void CountdownMenu() {
    ...
    while (true) {
        ...
        // --- 逻辑：当计时器正在运行或已暂停时 ---
            ...
            if (button_pressed) { // 短按按钮用于暂停/继续
                tone(BUZZER_PIN, 2000, 50);
                if (countdown_running) { // 如果正在运行 -> 暂停
        ...
        if (countdown_running) {
            ...
            if (millis_left == 0) {
                countdown_running = false;
                countdown_paused = false;
                tone(BUZZER_PIN, 3000, 3000); // 长鸣3秒作为结束提示
                displayCountdownTime(0);
            }
        } else { // 如果倒计时未运行（暂停或设置中），则只更新顶部的实时时钟
            unsigned long current_millis = millis();
            if (current_millis - last_display_update_time >= 1000) { // 每秒更新一次
                displayCountdownTime(countdown_duration_seconds * 1000);
                last_display_update_time = current_millis;
            }
        }
